...
# -------------------------
# Brand (minimal change) & Simple Product Catalog 
# -------------------------
BRAND_NAME = "Flipkart"  
...
async def show_catalog(
    ...
    if not prods:
        return f"Sorry — I couldn't find any items on {BRAND_NAME} that match. Would you like to try another search?"
    # Summarize top 8
    lines = [f"Here are the top {min(8, len(prods))} items I found on {BRAND_NAME}:"]
    ...
    # small 'prime' hint (demo)
    lines.append("Tip: You can say 'prime' to hear Prime-eligible items (demo only).")
    return "\n".join(lines)
...
async def show_catalog(
    ...
    if not prods:
        return f"Sorry — I couldn't find any items on {BRAND_NAME} that match. Would you like to try another search?"
    # Summarize top 4
    lines = [f"Here are the top {min(4, len(prods))} items I found on {BRAND_NAME}:"]
...
# -------------------------
# The Agent (Ramu Kaka) - minimal persona tweak for ecommerce
# -------------------------
class GameMasterAgent(Agent):
    def __init__(self):
        # System instructions now describe the shopkeeper persona and commerce role
        instructions = f"""
        You are 'Ramu Kaka', the friendly shopkeeper and voice assistant representing {BRAND_NAME}.
        Universe: A neighbourhood-style store experience mapped to a large e-commerce brand.
        Tone: Warm, helpful, slightly jocular; keep sentences short for TTS clarity.
        Role: Help the customer browse the catalog, add items to cart, place orders, and review recent orders on {BRAND_NAME}.
        ...
        Rules:
            ...
            - If user asks about 'prime' or 'fast delivery', explain that's a demo hint.
        """
        super().__init__(
...
async def entrypoint(ctx: JobContext):
    ...
    logger.info("\n" + "🛍️" * 6)
    logger.info(f"🚀 STARTING VOICE E-COMMERCE AGENT ({BRAND_NAME}) — Ramu Kaka")
